...
export default function PhoneDropDown({
  ...
  useEffect(() => {
    if (menuOpen == true) setFilter("");
  }, [menuOpen]);
  useEffect(() => {
    try {
      if (prevPhoneNumber.includes("-") && prevPhoneNumber.length > 3) {
        let splitArray = prevPhoneNumber.split("-");
    ...
    <div style={{ width: "100%", height: "86px" }}>
      ...
      <div
        className={`MainPhoneDropDown ${
          inputFouce && `MainPhoneDropDownFouce`
        } ${showError && `MainPhoneDropDownError`}`}
        ref={divRef}
        tabIndex={0}
        onBlur={handleBlur}
      ...
      >
        ...
        {showError && <label className={"errorLbl"}>{errorMsg}</label>}
        <div
          className={`PhoneDropDown_option dropdown_menu-6 ${
            menuOpen == true && "opening"
                    ...
                    if (
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            style={{ minWidth: "max-content", width: "80px" }}
                          >
