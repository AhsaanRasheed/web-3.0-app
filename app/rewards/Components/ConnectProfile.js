...
const ConnectProfile = ({ triggerRefreshStreaks }) => {
  ...
  const fetchTasksWithoutLoader = async () => {
    console.log("--fetchTasksWithoutLoader");
    try {
      const response = await get_rewards_tasksOneTime();
      const fetchedTasks = response.data;
      console.log("-- date fetchtaskswithoutloader", response.data);
      setTasks(response.data);
  ...
  const getIconButton = (taskName) => {
    switch (taskName.toLowerCase()) {
      ...
      case "add email and password":
        return <UpdateEmailPwdIcon />;
      default:
        return null;
    }
  };
  ...
  const getLinkForTask = (taskName) => {
    ...
    } else if (lowerCaseTaskName === "add email and password") {
      return "/update_x_email_pwd";
    } else {
      return "/";
    }
  };
  ...
  return (
    <div className={style.walletConnectContainer}>
      <div className={style.progressContainer}>
        <div className={style.title}>
          <div>Complete Profile</div>
          <div>{`${Number(Number(progress).toFixed(1))}%`}</div>
        </div>
        ...
        <div className={style.buttonsContainer}>
          {tasks?.map((task) => (
            <Link
              ...
              onClick={(e) => {
                if (
                  task.name.toLowerCase() == "connect x" &&
                  !task.is_completed
                ) {
                  e.stopPropagation();
                  e.preventDefault();
            ...
            >
              <div
                key={task.id}
                className={
                  task.is_completed ? style.buttonCompleted : style.button
                }
              >
...
const UpdateEmailPwdIcon = () => {
  return (
    <img src="data:image/png;base64,"></img>
  );
};
